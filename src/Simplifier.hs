...
-- Transform a RE into an RE that is in canonical form. So two REs which are equivalent should result in the same canonical forms.
mkCanon :: RE -> RE
mkCanon re  | re == re' = re
...
mkCanon' (CUnion CAny _) = CAny
mkCanon' (CUnion _ CAny) = CAny
